...
const CashflowSankey: React.FC<CashflowSankeyProps> = ({ transactions, expenseCategories, income, expenses }) => {
    ...
    const sankeyData = useMemo(() => {
        ...
        nodes.forEach((node) => {
            ...
            const totalOut = links.filter(l => l.source === nodeMap.get(node.name)).reduce((sum, l: { value: number }) => sum + l.value, 0);
            // FIX: Explicitly type 'l' in reduce callback to fix 'unknown' type error.
            const totalIn = links.filter(l => l.target === nodeMap.get(node.name)).reduce((sum, l: { value: number }) => sum + l.value, 0);
